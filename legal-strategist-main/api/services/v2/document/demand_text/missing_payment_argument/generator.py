...
class MissingPaymentArgumentGenerator(BaseGenerator):
    ...
    def _create_bill_template(self) -> str:
        ...
        if self.input.over_creditor:
            lines.append("La empresa {creditor.name}, con RUT {creditor.identifier}, ha cedido a mi representado, {over_creditor.name}, la factura electrónica Nº {identifier}, emitida con fecha {creation_date}, por un monto total de ${amount}.-")
        else:
            lines.append("La empresa {creditor.name}, con RUT {creditor.identifier}, ha emitido la factura electrónica Nº {identifier} con fecha {creation_date}, por un monto total de ${amount}.-")
        lines.extend([
            "Cabe señalar que la factura cedida cumple con lo dispuesto en la Ley 19.983, por lo que, para efectos de preparar la vía ejecutiva, procede que se notifique judicialmente a su obligado al pago.",
            "Asimismo, la cesión de las facturas fue debidamente notificada al deudor, {debtor.name}, tal como consta en el certificado de anotación en el Registro Público Electrónico de Transferencia de Créditos, llevado por el Servicio de Impuestos Internos.",
            "Sin embargo, el deudor {reason}. Por lo tanto, además del precio nominal de la prestación adeudada, que asciende a ${pending_amount}.-, aquel deberá pagar los intereses correspondientes, a contar de la mora o simple retardo en el pago del precio, y hasta la fecha de pago íntegro y total de lo adeudado.",
        ])
        return "\n\n".join(lines)
    ...
    def _create_promissory_note_template(self) -> str:
        lines: list[str] = []
        lines.extend([
            "Pagaré {identifier}, suscrito a la orden de {creditor.name}, con fecha {creation_date}, por la suma de ${amount}.-, que la parte demandada declaró haber recibido en préstamos en dineros en efectivo a su entera satisfacción y conformidad, obligándose a pagarlo conjuntamente con los intereses devengados, en {payment_installments} cuotas {payment_frequency}, iguales y sucesivas de ${amount_per_installment}.-, con excepción de la última cuota, que sería por la suma de ${amount_last_installment}.-, con vencimiento los días {due_payment_day} de cada {payment_frequency}, a contar de {first_installment_date}, y hasta {last_installment_date}.",
            "Se estipuló que el capital adeudado devengaría una tasa de interés del {interest_rate}% {interest_rate_frequency}, calculado en base a un año de {interest_rate_base_days} días y vencidos y el número días efectivamente transcurridos, el que regirá a contar de la fecha de suscripción y por todo el periodo de la obligación, pagaderos conjuntamente con el capital adeudado.",
            "Asimismo se pactó, que en caso de mora o simple retardo en el pago de capital o intereses, la tasa de interés se elevará al interés máximo convencional que la ley permita estipular para operaciones de crédito en moneda nacional, vigente a esta fecha o a la época de la mora o retardo a elección del acreedor, desde el momento del retardo y hasta el pago efectivo.",
            "Además se estipuló que el Banco podría hacer exigible el pago total de la suma de la deuda o del saldo a que ésta se hallara reducida, considerando la presente obligación como de plazo vencido, en caso de mora o simple retardo en el pago de una cualquiera de las cuotas en que se dividió la obligación, sea de capital y/o intereses, sean consecutivas o no, sin perjuicio de los demás derechos del acreedor.",
        ])
        ...
        lines.extend([
            "Por último, se pactó que todas las obligaciones que emanan del pagaré serán solidarias para el o los suscriptores, avalistas y demás obligados al pago y serán indivisibles para sus herederos y/o sucesores conforme a los artículos 1526 número 4 y 1528 del Código Civil. Además, cabe señalar que en el pagaré se liberó expresamente a {creditor.name} de la obligación de protesto.",
            "Es del caso que el deudor {reason}, razón por la cual y de acuerdo a lo estipulado vengo en hacer exigible, a contar de la fecha la presentación de la presente demanda, el total de la obligación insoluta, la que asciende a la suma de ${pending_amount}.-, de los cuales, ${capital_amount}.- corresponde a capital, ${interest_amount}.- corresponde a intereses, y ${debt_amount}.- corresponde a recargo por mora.",
            "La firma de los suscriptores del pagaré antes singularizado se encuentra autorizada por Notario Público, razón por la cual dicho instrumento tiene mérito ejecutivo respecto de todos sus obligados, y siendo las obligación líquida, actualmente exigible y estando la acción ejecutiva vigente, procede se despache mandamiento de ejecución y embargo en su contra.",
        ])
        return "\n\n".join(lines)
