    ...
    def generate_response(self, message: str) -> str | None:
        ...
        if legal_case and information_list:
            ...
            if request_llm.email_type == EmailType.DISPATCH_RESOLUTION:
                try:
                    dispatch_resolution_structure: DispatchResolutionStructure | None = case_tracker.create_dispatch_resolution_from_attachments(self.session, information_list)
                    if dispatch_resolution_structure is None:
                        errors.append("System could not store the dispatch resolution in our database, please contact the administrator about it")
                    else:
                        try:
                            statement = (
                                select(CaseEvent)
                                .where(CaseEvent.case_id == legal_case.id, CaseEvent.type == CaseEventType.DISPATCH_RESOLUTION, CaseEvent.simulated == False)
                                .order_by(desc(CaseEvent.created_at))
                            )
                            case_event = self.session.exec(statement).first()
                            if case_event is None:
                                raise ValueError("No CaseEvent found for the given criteria.")
                            suggestions = case_tracker.create_dispatch_resolution_suggestions(self.session, case_event, dispatch_resolution_structure)
                            ...
                            info.append(f"Consider: <resolution>{dispatch_resolution_structure.resolution}</resolution>")
                except Exception as e:
                ...
                try:
                    demand_text_structure: DemandExceptionStructure | None = case_tracker.create_demand_exception_from_attachments(self.session, information_list)
                    if demand_text_structure is None:
                        errors.append("System could not store the exceptions in our database, please contact the administrator about it")
                    else:
                        try:
                            statement = (
                                select(CaseEvent)
                                .where(CaseEvent.case_id == legal_case.id, CaseEvent.type == CaseEventType.EXCEPTIONS, CaseEvent.simulated == False)
                                .order_by(desc(CaseEvent.created_at))
                            )
                            case_event = self.session.exec(statement).first()
                            if case_event is None:
                                raise ValueError("No CaseEvent found for the given criteria.")
                            suggestions = case_tracker.create_demand_exception_suggestions(self.session, case_event, demand_text_structure)
                            if suggestions is None:
                            ...
                            info.append(f"Consider: <exceptions>{demand_text_structure.exceptions}</exceptions>")
                except Exception as e:
