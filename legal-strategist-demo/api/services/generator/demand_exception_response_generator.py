...
class DemandExceptionResponseGenerator(BaseGenerator):
    ...
    def generate(self, demand_exception: DemandExceptionStructure, demand_text: JudicialCollectionDemandTextStructure) -> LegalExceptionResponse:
        exception_extractor = DemandExceptionExtractor()
        ...
        exceptions = exception_extractor.extract_exceptions(demand_exception.exceptions) if demand_exception.exceptions else None
        exception_responses = self._generate_exception_responses(exceptions or [], demand_text_content)
