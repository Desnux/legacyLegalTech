...
export async function caseCreateFromDemandText(demandText: DemandTextGeneratorResponse) {
  const url = `${BACKEND_URL}/case/create-from-demand-text/`;
  const token = getAuthToken();
  ...
  try {
    ...
    console.error("Error sending case creation request:", error);
    throw new Error("No fue posible crear un caso asociado al texto de demanda.");
  }
}
