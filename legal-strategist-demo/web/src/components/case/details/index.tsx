...
import { fetchCaseDetails, fetchCaseEvents, fetchEventDocuments, fetchEventSuggestions, simulateFutureEvents } from "@/services/case";
...
const CaseDetails: React.FC<Props> = ({ caseId }) => {
  ...
  const simulateFuture = async () => {
    const simulation = await simulateFutureEvents(caseId);
    await loadCaseData();
  }
    ...
    <div className="p-6 md:p-8 bg-white md:rounded-xl md:shadow-lg md:mb-4 relative">
      ...
      >
        ...
        <div className="md:col-span-2" style={{ gridArea: "details" }}>
          <h1 className="text-xl md:text-2xl font-semibold mb-4">{caseData.title}</h1>
          <div className="mb-6">
        ...
        <div className="hidden lg:block" style={{ gridArea: "avenimiento" }}>
          <Advent
            ...
            lastEvent={relatedEvents[relatedEvents.length-1].type}
          ...
          {relatedEvents.map((event) => (
            <div key={event.id} className="mb-8 ml-2">
              ...
              <div className="ml-4 md:ml-6">
                <h3 className="text-base md:text-lg font-bold text-gray-800 mb-1">{event.title}</h3>
      ...
      <div className="absolute bottom-4 right-4 hidden md:block">
        <button
          type="button"
          className="opacity-50 hover:opacity-100 bg-white rounded-lg shadow-sm"
