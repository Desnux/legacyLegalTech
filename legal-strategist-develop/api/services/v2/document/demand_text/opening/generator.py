...
class DemandTextOpeningGenerator(BaseGenerator):
    ...
    def _create_template(self) -> str:
        ...
        document_count = self.input.document_count or 0
        lines.extend([
            "{creditor_sponsoring_attorney.name}, abogado, domiciliado en {creditor_sponsoring_attorney.address}, mandatario judicial, en representación convencional, según se acreditará, de {creditor.name}, {creditor_economic_activity}, Rol Único Tributario N° {creditor.identifier}, representado legalmente por {creditor_legal_representative.name}, cédula de identidad N° {creditor_legal_representative.identifier}, {creditor_legal_representative.occupation}, ambos con domicilio en {creditor_legal_representative.address + creditor.address}, a US. respetuosamente digo:",
            "Que en la representación que invisto, vengo en interponer la presente demanda ejecutiva en contra de {debtor.name}, {debtor_economic_activity}, Rol Único Tributario N° {debtor.identifier}, representada por {debtor.legal_representative.name}, {debtor.legal_representative.occupation}, {debtor.legal_representative.civil_status}, {debtor.legal_representative.nationality}, cédula de identidad N° {debtor.legal_representative.identifier}, ambos con domicilio para estos efectos en {debtor.legal_representative.address + debtor.address}, en su calidad de deudor principal, y en contra de {co_debtor.name}, ya individualizado, en su calidad de aval, fiador y codeudor solidario, en base de las consideraciones que a continuación paso a exponer:",
        ])
        if document_count > 1:
