...
class DispatchResolutionEventManager(
    ...
    def _process_information(
        ...
        if len(valid_case_events) == 0:
            raise ValueError("Case does not have dispatch start events.")
        ...
        if next_event:
            raise ValueError("Case does not have unresolved dispatch start events.")
        ...
        try:
            ...
            session.commit()
            # Resolver DISPATCH_START vinculándolo a DISPATCH_RESOLUTION
